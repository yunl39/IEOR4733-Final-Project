...
def form_portfolio(
    ...
    ----------
    pred_df : pd.DataFrame
        MultiIndex [date, ticker], columns [pred, realized].
    top_frac : float
        Fraction of stocks to long based on highest predicted returns.
    bot_frac : float
        Fraction of stocks to short based on lowest predicted returns.
    cost_bps : float
        One-way transaction cost in basis points.
        Cost is deducted only on rebalance days and scaled by turnover.
    holding_days : int
        Rebalance every N trading days. 1 = daily, 5 = approximately weekly.
...
def run_portfolio_comparison(
    ...
    top_frac: float = 0.20,
    bot_frac: float = 0.20,
    results_dir: str | Path = "results_portfolio",
    no_plot: bool = False,
):
    """
    ...
    for name, port in portfolios.items():
        metrics.append(compute_metrics(port.loc[common, "ls_ret"], f"{name} L/S net"))
        metrics.append(compute_metrics(port.loc[common, "ls_ret_gross"], f"{name} L/S gross"))
        metrics.append(compute_metrics(port.loc[common, "long_ret"], f"{name} Long"))
    ...
    for name, port in portfolios.items():
        returns_df[f"{name}_ls_net"] = port.loc[common, "ls_ret"]
        returns_df[f"{name}_ls_gross"] = port.loc[common, "ls_ret_gross"]
        returns_df[f"{name}_long"] = port.loc[common, "long_ret"]
        returns_df[f"{name}_short"] = port.loc[common, "short_ret"]
        returns_df[f"{name}_turnover"] = port.loc[common, "turnover"]
        returns_df[f"{name}_transaction_cost"] = port.loc[common, "transaction_cost"]

    returns_df.to_csv(results_dir / "daily_returns.csv")

    cumulative_cols = [c for c in returns_df.columns if "turnover" not in c and "transaction_cost" not in c]
...
def parse_args(argv=None):
    ...
    parser.add_argument(
        "--results_dir",
        type=str,
        default="results/transaction_costs",
