...
Outputs
-------
1. Long-short plots without LSTM (uWN / cWN / SPY)
2. Long-short plots with LSTM (uWN / cWN / LSTM / SPY)
3. Long-only plots with LSTM (uWN / cWN / LSTM / SPY)

Notes
-----
- SPY logic is kept unchanged: daily log return benchmark
- Target is also kept unchanged: forward 1-day log return
- No clipping in feature normalization
"""
...
def load_features(max_tickers: int | None = None) -> pd.DataFrame:
    ...
    # Keep original target logic unchanged: forward 1-day log return
...
def fit_normalize(X_train: np.ndarray):
    N, W, F = X_train.shape
...
def apply_normalize(X: np.ndarray, scaler: StandardScaler):
    N, W, F = X.shape
    X_norm = scaler.transform(X.reshape(-1, F)).reshape(N, W, F).astype(np.float32)
    return X_norm
...
class ConditionalTCNWrapper:
    ...
    """
    def __init__(self, n_stock_feats: int, n_market_feats: int, **model_kwargs):
...
def _make_loader(X, y, batch_size: int = 512, shuffle: bool = True):
    ds = TensorDataset(
...
@torch.no_grad()
def predict_torch(model_or_wrapper, X: np.ndarray, device: str = "cpu", batch_size: int = 1024) -> np.ndarray:
    if isinstance(model_or_wrapper, ConditionalTCNWrapper):
...
def form_portfolio(pred_df: pd.DataFrame, top_frac: float = 0.20, bot_frac: float = 0.20) -> pd.DataFrame:
    rows = []
...
def load_spy_returns(dates_needed) -> pd.Series:
    # Keep SPY logic unchanged: daily log return
...
def _rolling_sharpe(series: pd.Series, window: int = 63) -> pd.Series:
    return series.rolling(window).apply(
        lambda r: r.mean() / (r.std() + 1e-9) * np.sqrt(252),
        raw=True,
    )
...
def plot_four_panel(
    ...
    common = spy_ret.index.copy()
    for s in portfolios.values():
        common = common.intersection(s.index)

    spy_r = spy_ret.reindex(common)
    aligned = {k: v.reindex(common) for k, v in portfolios.items()}
    ...
    dd_spy = _drawdown(spy_r)
    ax3.fill_between(dd_spy.index, dd_spy.values, 0, alpha=0.28, label="SPY")
    ...
    all_series = {**aligned, "SPY": spy_r}
    annual_dict = {k: _annual_returns(v) for k, v in all_series.items()}
    ...
    for i, (name, annual_s) in enumerate(annual_dict.items()):
        vals = [annual_s[annual_s.index.year == y].iloc[0] if y in annual_s.index.year else 0.0 for y in years]
        offset = (i - (n_series - 1) / 2) * width
        ax4.bar(x + offset, vals, width=width, alpha=0.85, label=name, color=colors.get(name, None))
...
def print_metrics_table(port_u, port_c, port_lstm, spy_ret, save_dir: Path | None = None):
    common = (
        port_u.index
    ...
    metrics = [
        ...
        compute_metrics(port_u.loc[common, "long_ret"], "uWN (Long)"),
        compute_metrics(port_c.loc[common, "long_ret"], "cWN (Long)"),
        compute_metrics(port_lstm.loc[common, "long_ret"], "LSTM (Long)"),
    ...
    for m in metrics:
        print(
            f"{m['name']:<18} {m['cum_return']:>8.1f} {m['ann_return']:>8.1f} "
...
def main():
    ...
    if not args.no_plot:
        if args.save_plot:
            ...
            save_path_ls_without = base_path.with_name(f"{stem}_ls_without_lstm{suffix}")
            save_path_ls_with = base_path.with_name(f"{stem}_ls_with_lstm{suffix}")
            save_path_long = base_path.with_name(f"{stem}_long_with_lstm{suffix}")
        else:
            save_path_ls_without = out_dir / "backtest_ls_without_lstm.png"
            save_path_ls_with = out_dir / "backtest_ls_with_lstm.png"
            save_path_long = out_dir / "backtest_long_with_lstm.png"

        # 1) Long-short without LSTM
        plot_four_panel(
            portfolios={
                "uWN": port_u["ls_ret"],
                "cWN": port_c["ls_ret"],
            ...
            spy_ret=spy_ret,
            title="TCN-based Cross-Sectional Equity Backtest (Long-Short: uWN vs cWN)",
            save_path=save_path_ls_without,
        )

        # 2) Long-short with LSTM
        plot_four_panel(
            portfolios={
                "uWN": port_u["ls_ret"],
                "cWN": port_c["ls_ret"],
                "LSTM": port_lstm["ls_ret"],
            },
            spy_ret=spy_ret,
            title="TCN-based Cross-Sectional Equity Backtest (Long-Short: uWN vs cWN vs LSTM)",
            save_path=save_path_ls_with,
        )

        # 3) Long-only with LSTM
        plot_four_panel(
            portfolios={
                "uWN": port_u["long_ret"],
                "cWN": port_c["long_ret"],
                "LSTM": port_lstm["long_ret"],
            },
            spy_ret=spy_ret,
            title="TCN-based Cross-Sectional Equity Backtest (Long Portfolio: uWN vs cWN vs LSTM)",
