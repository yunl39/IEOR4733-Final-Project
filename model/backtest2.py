...
-----
    cd model/
    python backtest.py
    python backtest.py --tickers 50
    python backtest.py --iters 200 --tickers 50
"""
...
def load_features(max_tickers: int | None = None) -> pd.DataFrame:
    ...
    # Forward 1-day log return as target
...
def build_samples(df: pd.DataFrame, dates, feat_cols: list[str], window: int, subsample: float = 1.0):
    """
    For every (ticker, date t) pair where t is in `dates` and where we have
    at least `window` prior rows, build:
...
def fit_normalize(X_train):
    """
    Fit StandardScaler on flattened training data.
    """
    N, W, F = X_train.shape
    ...
    for epoch in range(n_iter):
        ...
        for xb, yb in loader:
            ...
            if not torch.isfinite(loss):
                print(f"  Warning: non-finite train loss at epoch {epoch}.")
                continue
...
class ConditionalTCNWrapper:
    """
    Wrap ConditionalTCNSeparate for stock/main features + market condition features.

    Main input:
        stock features only

    Condition list:
        each market feature becomes one separate condition series
    """
...
def train_conditional(
    ...
    for epoch in range(n_iter):
        ...
        for xb, yb in loader:
            ...
            if not torch.isfinite(loss):
                print(f"  Warning: non-finite conditional train loss at epoch {epoch}.")
                continue
...
def form_portfolio(pred_df: pd.DataFrame, top_frac=0.20, bot_frac=0.20) -> pd.DataFrame:
    """
    Given ['pred', 'realized'] indexed by (date, ticker), compute daily
...
def run_backtest(
    ...
    Walk-forward backtest. Returns:
        results_u : daily portfolio returns for uWN
        results_c : daily portfolio returns for cWN
    """
    ...
    for tr_dates, vl_dates, te_dates in rolling_windows(
        ...
        if verbose:
            ...
            print(f"  y_tr_n finite: {np.isfinite(y_tr_n).all()} | max abs: {np.max(np.abs(y_tr_n)):.4f}")
            print(f"  y_vl_n finite: {np.isfinite(y_vl_n).all()} | max abs: {np.max(np.abs(y_vl_n)):.4f}")
        ...
        chunk_u = pd.DataFrame({"pred": pred_u, "realized": realized}, index=idx)
        chunk_c = pd.DataFrame({"pred": pred_c, "realized": realized}, index=idx)
...
def plot_results(port_u: pd.DataFrame, port_c: pd.DataFrame, spy_ret: pd.Series, save_path: Path | None = None):
    ret_u = port_u["ls_ret"]
    ret_c = port_c["ls_ret"]
...
def print_metrics_table(port_u, port_c, spy_ret, save_dir: Path | None = None):
    common = port_u.index.intersection(port_c.index).intersection(spy_ret.index)
...
def parse_args():
    ...
    p.add_argument("--tickers", type=int, default=None, help="Max tickers to use")
    ...
    p.add_argument("--window", type=int, default=30, help="TCN lookback window")
    p.add_argument("--channels", type=int, default=32)
    p.add_argument("--layers", type=int, default=4)
    p.add_argument("--iters", type=int, default=700, help="Max training epochs per fold")
    ...
    p.add_argument("--results_dir", type=str, default="results2")
